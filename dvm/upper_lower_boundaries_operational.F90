...
module dvm_upper_lower_boundaries_operational
...
type, extends(type_base_model), public :: type_upper_lower_boundaries_operational

    type (type_dependency_id)                       :: id_par, id_migrator_food, id_depth 
...
contains
    ...
    subroutine initialize(self, configunit)
        class (type_upper_lower_boundaries_operational), intent(inout), target :: self
...
!        call self%register_dependency(self%id_topo,standard_variables%bottom_depth )
        ...
        call self%get_parameter(self%depth_threshold,'depth_threshold','m','depth where below migrator will not swim into',default=500.0_rk)
    end subroutine initialize

    subroutine do(self, _ARGUMENTS_DO_)

        class (type_upper_lower_boundaries_operational), intent(in) :: self
        _DECLARE_ARGUMENTS_DO_
    
        real(rk) :: par, par0, nhours, food
        real(rk) :: parlog, par0log
        real(rk) :: depth
